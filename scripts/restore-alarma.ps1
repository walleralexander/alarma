...
function Unprotect-Backup {
    ...
    try {
        if ($Passphrase) {
            ...
            $gpgCmd = "gpg --batch --yes --passphrase `"$Passphrase`" --output `"$decryptedFile`" --decrypt `"$EncryptedFile`""
        }
        else {
        ...
        if ($env:GPG_PASSPHRASE) {
            Remove-Item env:GPG_PASSPHRASE
        }
    }
}
...
function Create-PreRestoreBackup {
...
"@
        ...
        if (-not $SkipPreRestore) {
            $script:PreRestoreBackup = Create-PreRestoreBackup
        }
